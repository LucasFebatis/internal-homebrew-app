...
class CustomGitHubPrivateRepositoryReleaseDownloadStrategy < CustomGitHubPrivateRepositoryDownloadStrategy
  ...
  def parse_url_pattern
    url_pattern = %r{https://github.com/([^/]+)/([^/]+)/blob/master/([^/]+)/(\S+)}
  ...
  def _fetch(url:, resolved_url:)
    ...
    curl_download url, "--header", "Accept: application/vnd.github.v3.raw", "--header", "Authorization: token #{@github_token}", to: temporary_path
  end
